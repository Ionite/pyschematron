...
<sch:schema xmlns:sch="http://purl.oclc.org/dsdl/schematron" xml:lang="en">
  ...
  <sch:p>
    This schema can be used to validate SVRL documents, and provides examples
  ...
  <sch:pattern>
    ...
    report">
        A
        <sch:name />
        comes after a fired-rule, a failed-assert or a successful-
        report.
      </sch:assert>
    </sch:rule>
    <!-- Catch-all rule-->
  ...
  <sch:pattern abstract="true" id="requiredAttribute">
    ...
    <sch:rule context=" $context ">
      <sch:assert test="string-length( $attribute ) &gt; 0">
        ...
        element
        should have a
        <sch:value-of select="$attribute /name()" />
        attribute.
      </sch:assert>
    </sch:rule>
  </sch:pattern>
